...
export function FinanceProvider({ children }: { children: React.ReactNode }) {
  ...
  const addTransactionHandler = useCallback(async (data: Omit<Transaction, 'id' | 'createdAt'>) => {
    ...
    refreshData();
      ...
      console.error('Erro interceptado no FinanceContext:', error);
      toast({
        title: 'Bloqueio do Supabase (Nuvem)',
        description: `Erro exato: ${error.message}`,
        variant: 'destructive',
      });
    }
  ...
  const addCategoryHandler = useCallback(async (data: Omit<Category, 'id'>) => {
    ...
    try {
      ...
      console.error('Erro interceptado na Categoria:', error);
      toast({
        title: 'Bloqueio do Supabase (Nuvem)',
        description: `Erro exato: ${error.message}`,
        variant: 'destructive',
